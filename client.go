...
func NewClient(config *Config) (*Client, error){
	...
	for _, addr := range config.TrackerAddr {
		trackerPool, err := newConnPool(addr, config.MaxConnections)
		if err != nil {
			return nil, err
		}
...
func (c *Client) getStorageConn(storageInfo *storageInfo) (net.Conn, error) {
	...
	storagePool, err := newConnPool(storageInfo.addr, c.config.MaxConnections)
	if err != nil {
		c.storagePoolLock.Unlock()
		return nil, err
	}
